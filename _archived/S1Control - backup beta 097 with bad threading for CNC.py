...
versionNum = "v0.9.7"  # v0.9.6 was the first GeRDA-control version
...
class GerdaCNCController:
    ...
    def __init__(self, port="/dev/ttyUSB0", baudrate=115200, timeout=30) -> None:
        ...
        self.last_moved_to_xyz_position: tuple = (0, 0, 0)
        # get serial num letter for determining xyz offsets per instrument
        match instr_model:
            case "Titan":
                # TODO: set ACTUAL titan xyz offsets
                self.instr_offset_x: int = 10
                self.instr_offset_y: int = 10
                self.instr_offset_z: int = 10
                printAndLog(
        ...
        self.z_movedown_coord: int = 40
        self.z_liftup_coord: int = 5
    ...
    def sleep_command(self, sleep_time):
        with self.lock:
            printAndLog(f"CNC Sleeping for {sleep_time}s...")
            time.sleep(sleep_time)
            printAndLog("CNC Awake. Resuming...")

    def sleep_in_thread(self, sleep_time):
        # to avoid threaded tkinter stuff
        if self.thread and self.thread.is_alive():
            return  # A thread is already running, don't start another one

        self.thread = threading.Thread(target=self.sleep_command, args=(sleep_time,))
        self.thread.start()

    def move_to_xyz(self, x: int = None, y: int = None, z: int = None):
        ...
        (temp_x, temp_y, temp_z) = self.last_moved_to_xyz_position
        # build gcode command
        command = "G0"
        if x is not None:
            command += f" X{x}"
        ...
        if z is not None:
            command += f" Z{z}"
            temp_z = z
        printAndLog(f"CNC Moving To: {(temp_x, temp_y, temp_z)}")
        self.send_command_in_thread(command)
        # self.wait_for_threaded_command_completion()
        self.last_moved_to_xyz_position = (temp_x, temp_y, temp_z)

    def move_to_sample_safely(self, sample: GerdaSample):
        """Function to move from one sample to the next safely, by moving up on z first, then across to next sample xy, then down onto sample. AVOIDS DRAGGING. DOES NOT RETURN UNTIL MOVEMENT COMPLETE."""
        (current_x, current_y, current_z) = self.last_moved_to_xyz_position
        sample_x = sample.x_position
        sample_y = sample.y_position
        sample_z = self.z_movedown_coord
        printAndLog(
            f"CNC Moving safely to new sample {sample.name_or_note} @ {sample_x, sample_y, sample_z}, from {self.last_moved_to_xyz_position}"
        )
        if (sample_x, sample_y, sample_z) == self.last_moved_to_xyz_position:
        ...
        self.move_to_xyz(z=self.z_liftup_coord)
        self.wait_for_threaded_command_completion()
        printAndLog(f"CNC XY moving over new sample")
        self.move_to_xyz(x=sample_x, y=sample_y)
        self.wait_for_threaded_command_completion()
        printAndLog(f"CNC Z moving down onto sample")
        self.move_to_xyz(z=self.z_movedown_coord)
        self.wait_for_threaded_command_completion()
        return
    ...
    def home(self):
        ...
        self.last_moved_to_xyz_position = (0, 0, 0)
        return response

    def wait_for_threaded_command_completion(self):
        # wait for the event to be set, means the current step has been completed
        if self.thread:
            self.thread.join()  # Wait for the thread to complete
            self.thread = None  # Reset the thread for the next command
            self.event.wait()  # Wait for the event to be set

    def wait_for_assay_completion_command(self):
        """command to be ran by wait_for_assay_completion_in_thread. waits until it knows instr_assayisrunning == False."""
        with self.lock:
            time.sleep(1)
            while instr_assayisrunning:
                time.sleep(1)

    def wait_for_assay_completion_in_thread(self):
        if self.thread and self.thread.is_alive():
            return  # A thread is already running, don't start another one
        self.thread = threading.Thread(target=self.wait_for_assay_completion_command)
        self.thread.start()

    def start_sample_sequence(
        self,
        ...
        for sample_obj in sample_sequence.listofsampleobjects:
            ...
            printAndLog(f"GeRDA Moving to Sample: {sample_obj.name_or_note}")
            self.move_to_sample_safely(sample_obj)
            if sample_obj.optional_illumination_name == None:
                ...
                if illumination_obj != None:
                    ...
                    printAndLog(
                        "Custom Assay skipped due to invalid Illumination Name - '{sample_obj.optional_illumination_name}'"
                    )
            # after assay has been started, wait 1 second for important vars to catch up
            self.sleep_in_thread(sleep_time=1)
            self.wait_for_threaded_command_completion()
            # wait for assay complete
            self.wait_for_assay_completion_in_thread()
            # while instr_assayisrunning:
            #     # check if assay is running still, then exit if it has stopped.
            #     self.sleep_in_thread(sleep_time=1)
            #     self.wait_for_threaded_command_completion()

            # after assay has completed.
            self.sleep_in_thread(sleep_time=wait_time_between_samples_s)
            self.wait_for_threaded_command_completion()
            # then repeat!
...
def loadGerdaSampleListCSV_clicked() -> None:
    ...
    if file_path:
        ...
        if gerda_sample_sequence.init_sucessful:
            ...
            printAndLog(f"{gerda_sample_sequence.listofsamplenames}")
            # TODO: implement update ui stuff to recognise loaded sample list
            # TODO: un-disable start gerda scan button?
            pass
    else:
...
def gerdaCNC_StartSampleSequence_clicked() -> None:
    ...
    if gerdaCNC == None:
        ...
        gerdaCNC.start_sample_sequence(
...
if __name__ == "__main__":
    ...
    button_gerda_startsampleseq = ctk.CTkButton(
        ctrltabview.tab("GeRDA"),
        text="START SAMPLE SEQUENCE",
        command=gerdaCNC_StartSampleSequence_clicked,
        font=ctk_jbm12B,
    )
    button_gerda_startsampleseq.grid(
        row=5, column=0, columnspan=1, padx=4, pady=4, sticky=tk.NSEW
    )
